...
curativo = ["🔴Usaste la venda, tu vida está en: 100%🔴","🔴Usaste la venda, tu vida está en: 50%🔴","🔴Usaste la venda, tu vida está en: 60%🔴","🔴Usaste tu vida, la venda está en: 75%, tu vida está en: 90%🔴","🔴Usaste la venda, está en: 91%🔴"] 

bomba = ["💣🧟‍♂️ Le lanzaste una bomba a 1 jefe zombi 🧟‍♀️💣","💣🧟 Le lanzaste una bomba a 3 jefes zombi 🧟💣","💣🧟‍♂️ Le lanzaste una bomba a 2 jefes zombi 💣🧟‍♀️", "💣 🧟‍♂️ Le lanzaste una bomba a 7 jefes zombis 💣🧟‍♂️", "💣🧟 Le lanzaste una bomba a 4 jefes zombis 🧟💣"]
...
pescar = ["🥈GANASTE LA MEDALLA: PESCADOR DE PLATA🥈","🥉GANASTE LA MEDALLA: PESCADOR DE BRONCE🥉","🥉GANASTE LA MEDALLA: PESCADOR DE BRONCE🥉","🥉GANASTE LA MEDALLA: PESCADOR DE BRONCE🥉","🥉GANASTE LA MEDALLA: PESCADOR DE BRONCE🥉","🟡Evento: /carpa 🟡","⚫️Pescaste 3 veces Luna Nocturna⚫️(+150 PUNTOS)","⚫️Pescaste 2 veces Luna Nocturna⚫️(+100 PUNTOS)"," ⚫️Pescaste 1 vez Luna Nocturna⚫️(+50 PUNTOS)","🟡Pescaste 1 vez Luna Dorada Camarón 🟡 (PUNTOS MÚLTIPLES)","🟡Pescaste 1x Lenguado dorado🟡 (PUNTOS MÚLTIPLES)","🪼🌈Pescaste 1x Pulpo arcoíris🪼🌈 (PUNTOS EXTRA)","🐢Atrapaste 3x Tortugas 🐢 (PÉRDIDA DE PUNTOS)","🦑Atrapaste 1x Calamar gigante 🦑 (LEGENDARIO)","🦀Atrapaste 6x Cangrejos 🦀 (COMÚN)", "🦀Atrapaste 2x Cangrejos 🦀 (COMÚN)", "🦀Atrapaste 8x Cangrejos 🦀 (COMÚN)", "🪼Atrapaste 1x Pulpo marino🪼(ÉPICO)", "🦈Atrapaste 2x Tiburones🦈 (ÉPICO)", "🦈Pescaste 5x tiburones🦈 (ÉPICO)","🦈Pescaste 8x tiburones🦈 (ÉPICO)","🦈Pescaste 1x tiburón🦈 (ÉPICO)","🐠Pescaste 1x atún de mar🐠 (LEGENDARIO)", "🐠Pescaste 3x peces payaso🐠 (LEGENDARIO)","🐠Pescaste 3x atunes de mar🐠 (LEGENDARIO)","🐠Pescaste 1x pez payaso🐠 (LEGENDARIO)","🐠Pescaste 8x peces payaso🐠 (LEGENDARIO) )","🐠Pescaste 10x peces payaso🐠 (LEGENDARIO)","🐟Pescaste 1x salmón🐟 (RARO)","🧜🏼‍♀️Pescaste 5x Sirena🧜🏼‍♀️(ÉPICA)","🧜🏼‍ ♀️Atrapaste 2x Sirena🧜🏼‍♀️(ÉPICA)","🧜🏼‍♀️Atrapaste 1x Sirena🧜🏼‍♀️(ÉPICA)","🐟Atrapaste 3x Salmón🐟 (RARO)","🟡Atrapaste 1 x Tilapia Dourada🟡 (PUNTOS MÚLTIPLES)","☠️🐋Atrapaste 3x Ballenas muertas☠️🐋 (PÉRDIDA DE PUNTOS)","🐋Atrapaste 11x Ballenas marinas🐋(COMÚN)","🐋🌈Atrapaste 1x Arcoíris Ballena🌈 🐋 (PUNTOS EXTRA)","🥈GANASTE LA MEDALLA: PESCADOR DE PLATA🥈","🥇GANASTE LA MEDALLA: PESCADOR DE ORO🥇","🏅GANASTE LA MEDALLA: PESCADOR ESTRELLA🏅","💎Evento: /camarón 💎"]

class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("laboral")
        await self.highrise.walk_to(Position(16.5 , 0.0 , 17.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrar en la sala ")   
        await self.highrise.chat(f"🔞️Bienvenido {user.username} Usa: !emote list o 1-97 Para bailes y gestos ")

        await self.highrise.chat(f"🔞Utilice: /ayuda para más información. ")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username == "Joshlly":
              parts = message.split(" ")
              if len(parts) != 2:
              ...
              try:
                  ...
                  await self.highrise.chat("Cantidad no válida")
                  return
              # Checks if the bot has the amount
              bot_wallet = await self.highrise.get_wallet()
              bot_amount = bot_wallet.content[0].amount
              if bot_amount < amount:
                  await self.highrise.chat("No hay fondos suficientes")
                  return
              ...
              if bot_amount < total_tip_amount:
                  await self.highrise.chat("No hay fondos suficientes para dar propina a todos")
                  return
        ...
        if message.lower().startswith("!tipme ") and user.username== "Joshlly":
                try:
                    ...
                    if bot_amount < amount:
                        await self.highrise.chat("No hay fondos suficientes en la billetera del bot. ")
                        return
                    ...
                    if total > bot_amount:
                        await self.highrise.chat("No hay fondos suficientes para dar la propina especificada.")
                        return
                    # Send tip to the user who issued the command
                    for bar in tip:
                        await self.highrise.tip_user(user.id, bar)
                    await self.highrise.chat(f"Se le ha avisado  {amount_str}.")
                except (IndexError, ValueError):
                    await self.highrise.chat("El importe de la propina no es válido. Especifique un número válido.")
                    
        if message.startswith("/pez"):
            await self.highrise.send_whisper(user.id,"Estás pescando 🎣...")
          
        if message.lower() == "/pez":
        ...
        if message.lower() == "/casate conmigo?":
        ...
        if message.startswith("/carpa"):
           ...
           await self.highrise.send_whisper(user.id,"🟡Atrapaste 1 carpa dorada🟡 GANASTE LA MEDALLA: (MEGA PESCADOR)  ")
          
        if message.startswith("/camarón"):
           await self.highrise.react("clap",user.id)
           await self.highrise.send_whisper(user.id,"💎Atrapaste 1 camarón diamante💎GANASTE LA MEDALLA: (PESCADOR MAESTRO DIAMANTE)")                                
        if message.startswith("/curativo"):
           await self.highrise.react("heart",user.id)

        if message.startswith("/blindaje"):
           await self.highrise.react("heart",user.id)
           await self.highrise.send_whisper(user.id,f"@{user.username} 🛡 Usaste el escudo 🛡")
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username == "Joshlly":            await self.teleporter(message)
        ...
        if        message.startswith("-floor1") or message.startswith("Piso1") or message.startswith("piso1") or message.startswith("P1") or message.startswith("/piso1") or message.startswith("!piso1") or    message.startswith("/1") or message.startswith("!1") or                                 message.startswith("/p1") or message.startswith("!p1"):
          await self.highrise.teleport(user.id, Position(12.5 , 0.25 , 15.5 ))

        if        message.startswith("-floor1") or message.startswith("Piso2") or message.startswith("piso2") or message.startswith("P2") or message.startswith("/piso2") or message.startswith("!piso2") or    message.startswith("/2") or message.startswith("!2") or                                 message.startswith("/p2") or message.startswith("!p2"):
          await self.highrise.teleport(user.id, Position(13.5 , 10.0 , 14.0 ))
        ...
        if        message.startswith("!lista") or    message.startswith("!emote lista") or                                 message.startswith("!emote lista") or message.startswith("!list"):
        ...
        if        message.startswith("/lista") or     message.startswith("/emote list") or   message.startswith("/emoteall") or    message.startswith("!emoteall") or   message.startswith("/emote lista") or                                 message.startswith("/emote lista") or message.startswith("/list"):
            await self.highrise.send_emote("dance-floss")

        if        message.startswith("/pueblos") or      message.startswith("!pueblos"):
        ...
        if             message.startswith("!emotes") or message.startswith("/emotes"):
            ...
            await self.highrise.send_whisper(user.id,f"Emotes disponibles del número 1 al 97 ")

        if        message.startswith("Ayuda") or      message.startswith("/ayuda") or      message.startswith("!ayuda") or message.startswith("ayuda"):
            await self.highrise.chat(f"/emote lista | /pueblos | /emotes  | /casate conmigo? | /jugar /pez /userinfo @ | !emoteall | !tele @ | !summon @ | !kick @ | !tele z,y,x | !tele @ z,y,x | ")
            await self.highrise.chat(f"[Emote] All | !emote all [Emote] | !tipall [amount] | !tipme [amount] | !loop [nombre] | !loop [1-97][número]")        
        ...
        if              message.startswith("Carteira") or    message.startswith("Wallet") or    message.startswith("wallet") or  message.startswith("Billetera") or    message.startswith("billetera") or       message.startswith("carteira"):
          if user.username == "Joshlly":
            wallet = (await self.highrise.get_wallet()).content
            await self.highrise.send_whisper(user.id,f"CANTIDAD : {wallet[0].amount} {wallet[0].type}")
            await self.highrise.send_emote("dance-floss")
        ...
        if message.startswith("!kick"):
          if user.username == "Joshlly":
              ...
              await self.highrise.chat("No tienes permiso para utilizar este comando.")
              return
          #separete message into parts
          parts = message.split()
          #check if message is valid "kick @username"
          if len(parts) != 2:
              await self.highrise.chat("formato de prohibición incorrecto.")
              return
          ...
          if "user_id" not in locals():
              await self.highrise.chat("usuário não encontrado, por favor corrija a coordenada do código")
              return
          #kick user
          try:
          ...
          await self.highrise.chat(f"{username} Ele foi banido da sala!!")
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"🔞Adiós {user.username}! ")
        await self.highrise.send_emote("dance-floss")
