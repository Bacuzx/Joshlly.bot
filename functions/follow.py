...
async def follow(self: BaseBot, user: User, message: str) -> None:
    async def following_loop(self: BaseBot, user: User, message: str) -> None:
        if message.startswith("/following_loop"):
            await self.highrise.chat("Comando no válido, use /follow")
            return
        while True:
    ...
    for task in task_list:
        if task.get_name() == "following_loop":
            await self.highrise.chat("ya estoy siguiendo a alguien ")
            return
    ...
    await self.highrise.chat(f"estoy siguiendo   {user.username} 🚶‍♂️")
...
async def stop(self: BaseBot, user: User, message: str) -> None:
    ...
    for task in task_list:
        if task.get_name() == "following_loop":
            task.cancel()
            await self.highrise.chat(f"Dejar de seguir  {user.username}")
            return
    await self.highrise.chat("no sigo a nadie  ")
    return