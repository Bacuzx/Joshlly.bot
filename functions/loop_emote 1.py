    ...
    async def loop_emote(self: BaseBot, user: User, emote_name: str) -> None:
        ...
        for emote in emote_list:
            if emote[0].lower() == emote_name.lower():
                ...
                break
        if emote_id == "":
            await self.highrise.send_whisper(user.id,f"🚫🔄 @{user.username} Para detener el circuito, simplemente camine🔄🚫")
            return
        user_position = None
        user_in_room = False
        ...
        if user_position == None:
            ...
            return
        await self.highrise.send_whisper(user.id,f"👯🏻‍♂️🔄 @{user.username} estas en un bucle: {emote_name} 👯🏻‍♂️🔄")
