...
async def userinfo (self: BaseBot, user: User, message: str) -> None:
    ...
    parts = message.split(" ")
    if len(parts) != 2:
        await self.highrise.chat("Formato incorrecto, por favor utilice: /userinfo @username")
        return
    ...
    if user:
        ...
        await self.highrise.chat("Usuario no encontrado, por favor especifique un usuario válido")
        return
    ...
    try:
        ...
        last_login = "Último inicio de sesión no disponible"
    ...
    await self.highrise.chat(f"""Usuario: {username}\nNúmero de seguidores: {number_of_followers}\nnumero de amigos  : {number_of_friends}\nNúmero de siguiente : {number_of_folowing}\nSe unió a Highrise en : {joined_at}\nÚltimo inicio de sesión: {last_login}\nNúmero de publicaciones: {number_of_posts}\nMás me gusta en una publicación: {most_likes_post}""")